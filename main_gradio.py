...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Column(elem_id="col-container"):
        gr.Markdown("""Group of Upamanyu,Rolwin,Puneeth,Rohith.....
                    Made with love from DSU""",
                    elem_id="header")

        with gr.Row():
            with gr.Column():
                video_inp = gr.Video(label="video_input")
                gr.Markdown("👋 **Step1**: Select a video in Examples (bottom) or input youtube video_id in this textbox,", elem_id="hint")
                with gr.Row():
